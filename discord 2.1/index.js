      ...
      let valores = element.split('/')
        ...
        console.log(date)  
          const job = new cron.CronJob(date, function() {
            //ACA INSERTAMO EL CODIGO QUE MANDA EL RECORDATORIO
            fechasArray.splice(i, 1);
            const channelA = client.channels.cache.get('697302762135748639');
            channelA.send({
              embed: {
                color: 'GREEN',
                description: ('@everyone, hay clase de **' + materia + '**')
              }
            });
            dateSending = false
